...
The Extension Workshop combines three governed inputs:

- `ecosystem.json` owns MOD identity, bilingual copy, compatibility contracts, evidence claims,
  declared maintainer names, advisor relationships, and optional `compatibility.followup_url` links
  for owner-blocked graduation work.
- `plugin-workshop-metadata.json` is generated by `scripts/sync_plugin_workshop_metadata.py`; it
  refreshes public GitHub metrics and materializes the declared people/relationship data for
  rendering.
- `plugin-workload-navigation.json` owns the bilingual Workload taxonomy and the 19 MOD-to-Workload
  mappings used by the page filter and card labels.

Their Draft-07 schemas are `schemas/ecosystem_v1.schema.json`,
`schemas/plugin_workshop_metadata_v1.schema.json`, and
`schemas/plugin_workload_navigation_v1.schema.json`.
...
A component may declare `advisors: []` to state that no advisor role applies.
The sync preserves this explicit choice instead of inferring a relationship from people data.
The plugin page hides the advisor field for that component. Omitting `advisors` keeps the existing
inference and unknown-metadata behavior. `source_patch` delivery denotes a source mod, without
claiming an installable Python distribution or Extension Manager activation contract.
