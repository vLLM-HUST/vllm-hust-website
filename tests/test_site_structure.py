...
def test_homepage_presents_a_verified_serving_ecosystem() -> None:
    ...
    assert (
        "Typed runtime contracts. 24 audited MODs. Evidence before claims." in html_text
    )
    assert "类型化运行时契约、24 个已审计 MOD、证据先于结论。" in html_text
    assert "Domestic-compute inference engine" in site_js
    assert "面向国产算力的推理引擎" in site_js
    assert 'class="plugin-path"' in html_text
    assert "24 MODs, with compatibility and evidence made explicit." in html_text
    assert "24 个 MOD，明确展示兼容性与证据边界。" in html_text
