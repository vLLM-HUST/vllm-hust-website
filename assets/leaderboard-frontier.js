...
(() => {
    ...
    function chart(result,color) {
        ...
        for(const row of result.measured){
            ...
            const p=row.point,text=`${label(p)}${M.failedCorrectness(p)?` · ${t('failed')}`:''}${notExercised(p)?` · ${t('notExercised')}`:''} · ${parallel(p)} · C${p.load.concurrency??'—'}: ${t('x')} ${fmt(row.x)}, ${t('y')} ${fmt(row.y)}`;
    ...
    Promise.all([
        fetch('./data/leaderboard_frontier.json?v=qwen35-output-budget-curves-20260925',{cache:'no-cache'}).then(r=>{if(!r.ok)throw new Error('Snapshot unavailable');return r.json();}).then(M.validate),
